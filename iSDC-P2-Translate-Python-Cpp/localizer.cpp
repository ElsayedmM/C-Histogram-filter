...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	int a=h*w;
	float belief_per_cell=1.0/a;
	vector< vector <float> > newGrid(h, vector<float>(w, belief_per_cell));
	return newGrid;
}
...
vector <vector <float> > newGrid(height, vector <float> (width, 0.f));
	...
	float p_miss) 
{
	...
	vector< vector <float> > newGrid(h, vector<float>(w, 0.f));
	for (int i = 0; i < h; ++i) {
		for (int j = 0; j < w ; ++j) {
